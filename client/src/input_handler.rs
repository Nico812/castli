use common::GameId;
use common::courtyard::FacilityType;
use common::game_objs::GameObjE;
use common::units::UnitType;
use crossterm::event::{Event, KeyCode, KeyEventKind, KeyModifiers};
use std::collections::HashMap;
use tokio::sync::mpsc::UnboundedSender;

use crate::camera::{Camera, CameraLocation};
use crate::coord::TermCoord;
use crate::game_state::GameState;
use crate::renderer::ModPlayerInfoTab;
use crate::shutdown::{ShutdownChannel, ShutdownReason};
use crate::tui::{T2C, Tui};
use crate::ui_state::{FacilitySelection, Inspect, InteractTarget, UiMode, UiState, UnitSelection};
use common::GameCoord;
use common::r#const::{COURTYARD_COLS, COURTYARD_ROWS, MAP_COLS, MAP_ROWS};

pub struct InputHandler;

impl InputHandler {
    pub fn handle_input(
        event: &Event,
        tx: &UnboundedSender<T2C>,
        game_state: &mut GameState,
        ui_state: &mut UiState,
        shutdown: ShutdownChannel,
    ) {
        if let Event::Resize(col, row) = event {
            ui_state.term_size_change = Some(TermCoord::new(*row as usize, *col as usize));
            return;
        }

        let key = if let Event::Key(key) = event
            && key.kind == KeyEventKind::Press
        {
            key
        } else {
            return;
        };

        match key.code {
            KeyCode::Char('q') => {
                shutdown.shutdown(ShutdownReason::Key);
            }
            KeyCode::Char('y') => ui_state.tab = ModPlayerInfoTab::Castle,
            KeyCode::Char('x') => ui_state.tab = ModPlayerInfoTab::Logs,
            KeyCode::Char('c') => ui_state.tab = ModPlayerInfoTab::Debug,
            _ => {}
        }

        match ui_state.mode {
            UiMode::Std => match (key.code, key.modifiers) {
                (KeyCode::Char('M'), _) => {
                    Self::change_camera_location(
                        &mut ui_state.camera,
                        CameraLocation::WorldMap,
                        None,
                        tx,
                    );
                }
                (KeyCode::Char('m'), _) => {
                    Self::change_camera_location(
                        &mut ui_state.camera,
                        CameraLocation::Map,
                        None,
                        tx,
                    );
                }
                (KeyCode::Char('k'), _) => {
                    Self::change_camera_location(
                        &mut ui_state.camera,
                        CameraLocation::Courtyard,
                        None,
                        tx,
                    );
                }
                (KeyCode::Char('l'), _) => {
                    Self::toggle_inspect(ui_state);
                }
                (KeyCode::Up, KeyModifiers::NONE) => ui_state.move_camera(0, -1),
                (KeyCode::Down, KeyModifiers::NONE) => ui_state.move_camera(0, 1),
                (KeyCode::Right, KeyModifiers::NONE) => ui_state.move_camera(1, 0),
                (KeyCode::Left, KeyModifiers::NONE) => ui_state.move_camera(-1, 0),
                (KeyCode::Up, KeyModifiers::CONTROL) => ui_state.move_camera(0, -8),
                (KeyCode::Down, KeyModifiers::CONTROL) => ui_state.move_camera(0, 8),
                (KeyCode::Right, KeyModifiers::CONTROL) => ui_state.move_camera(8, 0),
                (KeyCode::Left, KeyModifiers::CONTROL) => ui_state.move_camera(-8, 0),
                _ => {}
            },
            UiMode::Inspect(ref mut inspect) => match (key.code, key.modifiers) {
                (KeyCode::Esc, _) => ui_state.mode = UiMode::Std,
                (KeyCode::Char('M'), _) => {
                    Self::change_camera_location(
                        &mut ui_state.camera,
                        CameraLocation::WorldMap,
                        Some(inspect),
                        tx,
                    );
                }
                (KeyCode::Char('m'), _) => {
                    Self::change_camera_location(
                        &mut ui_state.camera,
                        CameraLocation::Map,
                        Some(inspect),
                        tx,
                    );
                }
                (KeyCode::Char('k'), _) => {
                    Self::change_camera_location(
                        &mut ui_state.camera,
                        CameraLocation::Courtyard,
                        Some(inspect),
                        tx,
                    );
                }
                (KeyCode::Char('l'), _) => {
                    Self::toggle_inspect(ui_state);
                }
                (KeyCode::Enter, _) => match ui_state.camera.location {
                    CameraLocation::Map | CameraLocation::WorldMap => {
                        let in_world_map = ui_state.camera.location == CameraLocation::WorldMap;

                        if game_state.player.castle_id.is_none() {
                            if !in_world_map {
                                Self::handle_new_castle(tx, inspect)
                            }
                            return;
                        }
                        let looked_objs =
                            Tui::get_looked_objs(inspect.coord, &game_state.objs, &ui_state.camera);

                        if looked_objs.len() > 1 && inspect.selection.is_none() {
                            inspect.selection = Some(looked_objs[0].0);
                        } else {
                            let interact_target = match looked_objs.len() {
                                0 => InteractTarget::MapPos(inspect.coord),
                                1 => InteractTarget::GameObj(looked_objs[0].0),
                                _ => InteractTarget::GameObj(inspect.selection.unwrap()),
                            };

                            ui_state.mode = UiMode::Interact(interact_target);
                        }
                    }
                    CameraLocation::Courtyard => {
                        if game_state.player.castle_id.is_none() {
                            return;
                        }
                        let looked_facility =
                            Tui::get_looked_facility(inspect.coord, &game_state.facilities);
                        if let Some(looked_facility) = looked_facility {
                            ui_state.mode =
                                UiMode::Interact(InteractTarget::Facility(*looked_facility.0));
                        } else {
                            ui_state.mode =
                                UiMode::Interact(InteractTarget::CourtyardPos(inspect.coord));
                        }
                    }
                },
                (KeyCode::Up, KeyModifiers::NONE) => {
                    Self::move_inspect(0, -2, inspect, &game_state.objs, &ui_state.camera)
                }
                (KeyCode::Down, KeyModifiers::NONE) => {
                    Self::move_inspect(0, 2, inspect, &game_state.objs, &ui_state.camera)
                }
                (KeyCode::Right, KeyModifiers::NONE) => {
                    Self::move_inspect(2, 0, inspect, &game_state.objs, &ui_state.camera)
                }
                (KeyCode::Left, KeyModifiers::NONE) => {
                    Self::move_inspect(-2, 0, inspect, &game_state.objs, &ui_state.camera)
                }
                (KeyCode::Up, KeyModifiers::CONTROL) => {
                    Self::move_inspect(0, -8, inspect, &game_state.objs, &ui_state.camera)
                }
                (KeyCode::Down, KeyModifiers::CONTROL) => {
                    Self::move_inspect(0, 8, inspect, &game_state.objs, &ui_state.camera)
                }
                (KeyCode::Right, KeyModifiers::CONTROL) => {
                    Self::move_inspect(8, 0, inspect, &game_state.objs, &ui_state.camera)
                }
                (KeyCode::Left, KeyModifiers::CONTROL) => {
                    Self::move_inspect(-8, 0, inspect, &game_state.objs, &ui_state.camera)
                }
                _ => {}
            },
            UiMode::Interact(ref mut interact_target) => match (key.code, key.modifiers) {
                (KeyCode::Esc, _) => ui_state.mode = UiMode::Std,
                (KeyCode::Char('a'), _) => match interact_target {
                    InteractTarget::MapPos(_) | InteractTarget::GameObj(_) => {
                        ui_state.mode = UiMode::UnitSelection(UnitSelection::from_interact(
                            interact_target.clone(),
                        ))
                    }
                    _ => {}
                },
                (KeyCode::Char('n'), _) => {
                    if let InteractTarget::CourtyardPos(pos) = interact_target {
                        ui_state.mode = UiMode::FacilitySelection(FacilitySelection::new(*pos));
                    }
                }
                _ => {}
            },
            UiMode::UnitSelection(ref mut selection) => {
                let Some(ref castle) = game_state.castle else {
                    return;
                };

                match (key.code, key.modifiers) {
                    (KeyCode::Esc, _) => ui_state.mode = UiMode::Std,
                    (KeyCode::Char('a'), _) => {
                        Self::handle_unit_deploy(tx, selection);
                        ui_state.mode = UiMode::Std;
                    }
                    (KeyCode::Enter, _) => {
                        if let Some(ref string) = selection.active_input.1 {
                            let unit_index = selection.active_input.0.as_index();

                            selection.selected_units.quantities[unit_index] = string
                                .parse()
                                .unwrap_or(0)
                                .min(castle.units.quantities[unit_index]);
                            selection.active_input.1 = None;
                        } else {
                            selection.active_input.1 = Some(String::new());
                        }
                    }
                    (KeyCode::Char(c), _) if c.is_ascii_digit() => {
                        if let Some(ref mut string) = selection.active_input.1 {
                            string.push(c);
                        }
                    }
                    (KeyCode::Backspace, _) => {
                        if let Some(ref mut string) = selection.active_input.1 {
                            let _ = string.pop();
                        }
                    }
                    (KeyCode::Up, KeyModifiers::NONE) => Self::move_unit_selection(-1, selection),
                    (KeyCode::Down, KeyModifiers::NONE) => Self::move_unit_selection(1, selection),
                    (KeyCode::Up, KeyModifiers::CONTROL) => {
                        Self::move_unit_selection(-8, selection)
                    }
                    (KeyCode::Down, KeyModifiers::CONTROL) => {
                        Self::move_unit_selection(8, selection)
                    }
                    _ => {}
                }
            }
            UiMode::FacilitySelection(ref mut selection) => {
                let Some(_) = game_state.castle else {
                    return;
                };
                match (key.code, key.modifiers) {
                    (KeyCode::Esc, _) => ui_state.mode = UiMode::Std,
                    (KeyCode::Enter, _) => {
                        let _ = tx.send(T2C::NewFacility((selection.pos, selection.active)));
                        ui_state.mode = UiMode::Inspect(Inspect {
                            coord: selection.pos,
                            selection: None,
                        });
                    }
                    (KeyCode::Up, KeyModifiers::NONE) => {
                        Self::move_facility_selection(-1, selection)
                    }
                    (KeyCode::Down, KeyModifiers::NONE) => {
                        Self::move_facility_selection(1, selection)
                    }
                    _ => {}
                }
            }
        }
    }

    fn handle_new_castle(tx: &tokio::sync::mpsc::UnboundedSender<T2C>, inspect: &Inspect) {
        let _ = tx.send(T2C::NewCastle(inspect.coord));
    }

    fn change_camera_location(
        camera: &mut Camera,
        new: CameraLocation,
        mut inspect: Option<&mut Inspect>,
        tx: &UnboundedSender<T2C>,
    ) {
        if camera.location == new {
            return;
        }

        if new == CameraLocation::Courtyard {
            let _ = tx.send(T2C::InCourtyard);
        };

        if camera.location == CameraLocation::Courtyard {
            let _ = tx.send(T2C::OutCourtyard);
        };

        if new == CameraLocation::Courtyard
            && let Some(ref mut inspect) = inspect
        {
            inspect.coord = GameCoord { x: 0, y: 0 };
        };
        if new == CameraLocation::WorldMap
            && camera.location == CameraLocation::Map
            && let Some(ref mut inspect) = inspect
        {
            inspect.coord.y -= inspect.coord.y % camera.zoom_factor;
            inspect.coord.x -= inspect.coord.x % camera.zoom_factor;
        }
        camera.location = new;
    }

    fn toggle_inspect(ui_state: &mut UiState) {
        if let UiMode::Std = ui_state.mode {
            let coord = match ui_state.camera.location {
                CameraLocation::WorldMap => GameCoord { x: 0, y: 0 },
                CameraLocation::Map => ui_state.camera.map,
                CameraLocation::Courtyard => ui_state.camera.courtyard,
            };
            ui_state.mode = UiMode::Inspect(Inspect {
                coord,
                selection: None,
            });
        } else if let UiMode::Inspect(_) = ui_state.mode {
            ui_state.mode = UiMode::Std;
        }
    }

    fn handle_unit_deploy(tx: &tokio::sync::mpsc::UnboundedSender<T2C>, selection: &UnitSelection) {
        match selection.interact_target {
            InteractTarget::GameObj(obj_id) => {
                let _ = tx.send(T2C::AttackCastle(obj_id, selection.selected_units.clone()));
            }
            InteractTarget::MapPos(pos) => {
                let _ = tx.send(T2C::SendUnits(pos, selection.selected_units.clone()));
            }
            _ => {}
        };
    }

    fn move_inspect(
        mut dx: isize,
        mut dy: isize,
        inspect: &mut Inspect,
        objs: &HashMap<GameId, GameObjE>,
        camera: &Camera,
    ) {
        if let Some(ref mut selection) = inspect.selection {
            let looked_objs = Tui::get_looked_objs(inspect.coord, objs, camera);
            let new_selection = {
                let current_pos = looked_objs.iter().position(|(id, _)| *id == *selection);
                match dy {
                    dy if dy > 0 => current_pos.and_then(|pos| {
                        let new_pos = (pos + 1).min(looked_objs.len() - 1);
                        looked_objs.get(new_pos).map(|(id, _)| *id)
                    }),
                    dy if dy < 0 => current_pos.and_then(|pos| {
                        let new_pos = pos.saturating_sub(1);
                        looked_objs.get(new_pos).map(|(id, _)| *id)
                    }),
                    _ => None,
                }
            };

            if let Some(new_id) = new_selection {
                *selection = new_id;
            }
        } else {
            match camera.location {
                CameraLocation::WorldMap => {
                    dx *= camera.zoom_factor as isize;
                    dy *= camera.zoom_factor as isize;
                    inspect.coord.x = (inspect.coord.x as isize + dx)
                        .max(0)
                        .min(MAP_COLS as isize - 1) as usize;
                    inspect.coord.y = (inspect.coord.y as isize + dy)
                        .max(0)
                        .min(MAP_ROWS as isize - 1) as usize;
                }
                CameraLocation::Map => {
                    inspect.coord.x = (inspect.coord.x as isize + dx)
                        .max(camera.map.x as isize)
                        .min((camera.map.x + camera.fov_size.x - 1) as isize)
                        as usize;
                    inspect.coord.y = (inspect.coord.y as isize + dy)
                        .max(camera.map.y as isize)
                        .min((camera.map.y + camera.fov_size.y * 2 - 1) as isize)
                        as usize;
                }
                CameraLocation::Courtyard => {
                    inspect.coord.x = (inspect.coord.x as isize + dx)
                        .max(camera.courtyard.x as isize)
                        .min(
                            (camera.courtyard.x + camera.fov_size.x.min(COURTYARD_COLS) - 1)
                                as isize,
                        ) as usize;
                    inspect.coord.y = (inspect.coord.y as isize + dy)
                        .max(camera.courtyard.y as isize)
                        .min(
                            (camera.courtyard.y + (camera.fov_size.y * 2).min(COURTYARD_ROWS) - 1)
                                as isize,
                        ) as usize;
                }
            }
            inspect.coord.x -= inspect.coord.x % 2;
            inspect.coord.y -= inspect.coord.y % 2;
        }
    }

    fn move_unit_selection(dy: isize, selection: &mut UnitSelection) {
        if selection.active_input.1.is_some() {
            return;
        }

        match dy {
            dy if dy > 0 => {
                let new_unit_index =
                    (selection.active_input.0.as_index() + 1).min(UnitType::COUNT - 1);
                selection.active_input.0 = UnitType::from_index(new_unit_index);
            }
            dy if dy < 0 => {
                let new_unit_index = selection.active_input.0.as_index().saturating_sub(1);
                selection.active_input.0 = UnitType::from_index(new_unit_index);
            }
            _ => {}
        }
    }

    fn move_facility_selection(dy: isize, selection: &mut FacilitySelection) {
        match dy {
            dy if dy > 0 => {
                let new_facility_index =
                    (selection.active.as_index() + 1).min(FacilityType::COUNT - 1);
                selection.active = FacilityType::from_index(new_facility_index);
            }
            dy if dy < 0 => {
                let new_facility_index = selection.active.as_index().saturating_sub(1);
                selection.active = FacilityType::from_index(new_facility_index);
            }
            _ => {}
        }
    }
}
