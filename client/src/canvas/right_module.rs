...
use common::{CastleE, GameObjE, StructureE, UnitGroupE};
...
impl RightModule {
    ...
    pub fn get_renderable_and_update(&self, frame_dt: u64, sel_obj: &GameObjE) -> Vec<Vec<TermCell>> {
        let mut content = vec![
        ...
        match sel_obj {
            GameObjE::Castle(castle) => {
                self.draw_text(&mut content, "--- Castle ---", current_row, Self::PADDING_LEFT);
                current_row += 2;

                let name_str = format!("Name: {}", castle.name);
                self.draw_text(&mut content, &name_str, current_row, Self::PADDING_LEFT);
                current_row += 1;

                let pos_str = format!("Position: ({}, {})", castle.pos.0, castle.pos.1);
                self.draw_text(&mut content, &pos_str, current_row, Self::PADDING_LEFT);
            }
            GameObjE::Structure(structure) => {
                self.draw_text(&mut content, "--- Structure ---", current_row, Self::PADDING_LEFT);
                current_row += 2;
                ...
                self.draw_text(&mut content, &type_str, current_row, Self::PADDING_LEFT);
                current_row += 1;
            ...
            GameObjE::UnitGroup(unit_group) => {
                self.draw_text(&mut content, "--- Unit Group ---", current_row, Self::PADDING_LEFT);
                current_row += 2;

                let owner_str = format!("Owner: {}", unit_group.owner);
                self.draw_text(&mut content, &owner_str, current_row, Self::PADDING_LEFT);
            }
        }
